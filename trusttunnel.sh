...
UPSTREAM_SERVICE_TEMPLATE="${INSTALL_DIR}/trusttunnel.service.template"
FOUND_CERT_DOMAINS=()
FOUND_CERT_PATHS=()
FOUND_KEY_PATHS=()
FOUND_CERT_SOURCES=()
...
install_trusttunnel() {
    ...
    echo "Downloading TrustTunnel ${TRUSTTUNNEL_VERSION}..."
    echo ""
    # Download and install using the official upstream installer with a pinned
    # version and explicit output directory so the configuration paths in this
    # script are guaranteed to match.
    curl -fsSL "${UPSTREAM_INSTALLER_URL}" \
        | sh -s -- -a y -V "${TRUSTTUNNEL_VERSION}" -o "${INSTALL_DIR}"
    if is_installed; then
        echo "TrustTunnel ${TRUSTTUNNEL_VERSION} installed successfully to ${INSTALL_DIR}"
    else
...
EOF
    ...
    echo ""
    echo "Diagnostic endpoints (TrustTunnel ${TRUSTTUNNEL_VERSION}+)"
    read -r -p "Enable HTTP /ping handler? [y/N]: " ENABLE_PING_INPUT
    ENABLE_PING_INPUT=$(echo "${ENABLE_PING_INPUT:-n}" | tr '[:upper:]' '[:lower:]')
    [[ "$ENABLE_PING_INPUT" == "y" || "$ENABLE_PING_INPUT" == "yes" ]] && PING_ENABLED="true" || PING_ENABLED="false"
    read -r -p "Enable HTTP /speedtest handler? [y/N]: " ENABLE_SPEEDTEST_INPUT
    ENABLE_SPEEDTEST_INPUT=$(echo "${ENABLE_SPEEDTEST_INPUT:-n}" | tr '[:upper:]' '[:lower:]')
    [[ "$ENABLE_SPEEDTEST_INPUT" == "y" || "$ENABLE_SPEEDTEST_INPUT" == "yes" ]] && SPEEDTEST_ENABLED="true" || SPEEDTEST_ENABLED="false"
    echo ""
    echo "Per-client connection limits (leave blank to disable)"
    read_default DEFAULT_MAX_HTTP2_CONNS "Default max HTTP/2 connections per client (or press enter for [unlimited]): " ""
    read_default DEFAULT_MAX_HTTP3_CONNS "Default max HTTP/3 connections per client (or press enter for [unlimited]): " ""
    if [[ -n "${DEFAULT_MAX_HTTP2_CONNS}" && ! "${DEFAULT_MAX_HTTP2_CONNS}" =~ ^[0-9]+$ ]]; then
        echo "Error: HTTP/2 connection limit must be a non-negative integer"
        exit 1
    fi
    if [[ -n "${DEFAULT_MAX_HTTP3_CONNS}" && ! "${DEFAULT_MAX_HTTP3_CONNS}" =~ ^[0-9]+$ ]]; then
        echo "Error: HTTP/3 connection limit must be a non-negative integer"
        exit 1
    fi
    cat << 'EOF'
...
Configuration Summary:
  ...
  Ping Handler:      ${PING_ENABLED}
  Speedtest Handler: ${SPEEDTEST_ENABLED}
  Default H2 Conns:  ${DEFAULT_MAX_HTTP2_CONNS:-unlimited}
  Default H3 Conns:  ${DEFAULT_MAX_HTTP3_CONNS:-unlimited}
...
# Generated by trusttunnel.sh (thekhabaroff fork) for TrustTunnel ${TRUSTTUNNEL_VERSION}.
...
speedtest_enable = ${SPEEDTEST_ENABLED}
speedtest_path = "/speedtest"
ping_enable = ${PING_ENABLED}
ping_path = "/ping"
# HTTP status code returned on auth failure (407 or 405); added in 1.0.17.
auth_failure_status_code = 407
EOF
    if [[ -n "${DEFAULT_MAX_HTTP2_CONNS}" ]]; then
        echo "default_max_http2_conns_per_client = ${DEFAULT_MAX_HTTP2_CONNS}" >> "${CONFIG_DIR}/vpn.toml"
    fi
    if [[ -n "${DEFAULT_MAX_HTTP3_CONNS}" ]]; then
        echo "default_max_http3_conns_per_client = ${DEFAULT_MAX_HTTP3_CONNS}" >> "${CONFIG_DIR}/vpn.toml"
    fi
    cat >> "${CONFIG_DIR}/vpn.toml" << 'EOF'
...
# TrustTunnel client credentials. Per-client overrides:
...
# Connection filtering rules for TrustTunnel.
# Each rule has an `action` ("allow" or "deny") and optional matchers:
#   cidr = "192.168.0.0/16"
#   client_random_prefix = "aabbcc"            # exact hex prefix (1.0.1+)
#   client_random_prefix = "a0b0/f0f0"         # bitwise prefix/mask
...
setup_systemd_service() {
    ...
    # Prefer the upstream-shipped systemd unit template; it tracks changes
    # to ExecStart flags between releases. Fall back to an embedded template
    # if the upstream installer didn't provide one (e.g. legacy installs).
    if [[ -f "${UPSTREAM_SERVICE_TEMPLATE}" ]]; then
        cp "${UPSTREAM_SERVICE_TEMPLATE}" "${SERVICE_FILE}"
        echo "Systemd service file installed from ${UPSTREAM_SERVICE_TEMPLATE}"
    else
...
EOF
        echo "Systemd service file created (embedded template)"
    fi
...
show_client_config() {
    ...
    echo ""
    # Since 1.0.28 the endpoint can auto-generate a `client_random_prefix`,
    # append a matching `allow` rule to rules.toml and embed it in the export.
    # Since 1.0.13 the deep-link format is `tt://?...` (the older `tt://` is
    # still accepted for backward compatibility).
    if [[ -f "${INSTALL_DIR}/trusttunnel_endpoint" ]]; then
        ${INSTALL_DIR}/trusttunnel_endpoint vpn.toml hosts.toml \
            -c "${VPN_USERNAME}" -a "${client_address}" \
            --client-random-prefix 2>/dev/null || \
        ${INSTALL_DIR}/trusttunnel_endpoint vpn.toml hosts.toml \
            -c "${VPN_USERNAME}" -a "${client_address}" 2>/dev/null || {
    ...
    echo "Deep-link format: tt://?<base64> (compatible with TrustTunnel 1.0.13+)"
}
show_current_configuration() {
    ...
    local ping_enabled=""
    local speedtest_enabled=""
    local default_max_http2=""
    local default_max_http3=""
    local auth_failure_status_code=""
    local vpn_username=""
    local vpn_password=""
    local protocols=""
    ...
    if [[ -f "${CONFIG_DIR}/vpn.toml" ]]; then
        ...
        ping_enabled=$(sed -n 's/^[[:space:]]*ping_enable[[:space:]]*=[[:space:]]*\(true\|false\).*/\1/p' "${CONFIG_DIR}/vpn.toml" | head -n1)
        speedtest_enabled=$(sed -n 's/^[[:space:]]*speedtest_enable[[:space:]]*=[[:space:]]*\(true\|false\).*/\1/p' "${CONFIG_DIR}/vpn.toml" | head -n1)
        default_max_http2=$(sed -n 's/^[[:space:]]*default_max_http2_conns_per_client[[:space:]]*=[[:space:]]*\([0-9]*\).*/\1/p' "${CONFIG_DIR}/vpn.toml" | head -n1)
        default_max_http3=$(sed -n 's/^[[:space:]]*default_max_http3_conns_per_client[[:space:]]*=[[:space:]]*\([0-9]*\).*/\1/p' "${CONFIG_DIR}/vpn.toml" | head -n1)
        auth_failure_status_code=$(sed -n 's/^[[:space:]]*auth_failure_status_code[[:space:]]*=[[:space:]]*\([0-9]*\).*/\1/p' "${CONFIG_DIR}/vpn.toml" | head -n1)
    ...
    if [[ -x "${INSTALL_DIR}/trusttunnel_endpoint" ]]; then
        installed_version=$("${INSTALL_DIR}/trusttunnel_endpoint" --version 2>/dev/null \
    ...
    echo "  Ping Handler   : ${ping_enabled:-N/A}"
    echo "  Speedtest      : ${speedtest_enabled:-N/A}"
    echo "  Auth Fail Code : ${auth_failure_status_code:-N/A}"
    echo "  Default H2 Cap : ${default_max_http2:-unlimited}"
    echo "  Default H3 Cap : ${default_max_http3:-unlimited}"
    echo "  Username       : ${vpn_username:-N/A}"
    echo "  Password       : ${vpn_password:-N/A}"
    echo "  Endpoint Build : ${installed_version:-${TRUSTTUNNEL_VERSION}}"
}
# ============================================================================
# Menu Functions
# ============================================================================
...
menu_add_user() {
    ...
    local new_max_http2=""
    local new_max_http3=""
    read -r -p "Per-user max HTTP/2 connections (blank = inherit default): " new_max_http2
    read -r -p "Per-user max HTTP/3 connections (blank = inherit default): " new_max_http3
    if [[ -n "${new_max_http2}" && ! "${new_max_http2}" =~ ^[0-9]+$ ]]; then
        echo "Error: HTTP/2 connection limit must be a non-negative integer"
        return
    fi
    if [[ -n "${new_max_http3}" && ! "${new_max_http3}" =~ ^[0-9]+$ ]]; then
        echo "Error: HTTP/3 connection limit must be a non-negative integer"
        return
    fi
    {
    ...
    [[ -n "${new_max_http2}" ]] && echo "Max HTTP/2 conns: ${new_max_http2}"
    [[ -n "${new_max_http3}" ]] && echo "Max HTTP/3 conns: ${new_max_http3}"
...
first_time_install() {
    ...
    cat << EOF
Welcome to TrustTunnel VPN Manager (thekhabaroff fork).
This wizard installs TrustTunnel ${TRUSTTUNNEL_VERSION} and walks you through
generating vpn.toml/hosts.toml/credentials.toml/rules.toml.
...
main() {
    echo ""
    echo ""
    print_with_delay "trusttunnel.sh installer/manager (thekhabaroff fork) | TrustTunnel ${TRUSTTUNNEL_VERSION}" 0.05
    echo ""
    echo ""
    check_root
    check_os
